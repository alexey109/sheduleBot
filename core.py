...
def isWeeksEqual(doc_week, cal_week):
    ...
    :rtype: bool
    """
    cal_week = cal_week - dt.date((2020, 2, 10)).isocalendar()[1] + 1
...
def cmdWeek(params):
    ...
    weeks = params['date'].date().isocalendar()[1] - dt.date((2020, 2, 10)).isocalendar()[1] + 1
                ...
                try:
                    ...
                    for idx, name in enumerate(CONST.MONTH_NAMES):
                        if re.search(name, month):
                            mnumb = idx + 1
                            break
                    date = dt.date(2020, mnumb, int(day))
                except:
                    del markers[cmd_code]
